...
private fun conformArrayObjectsToConstructor(inputArray: List<Any?>, parameters: List<KParameter>) =
    inputArray.mapIndexed { index, item ->
        if (isValidValueForParameter(item, parameters[index]))
            item
        else
            tryToConvert(item, parameters[index])
    }

private fun isValidValueForParameter(value: Any?, parameter: KParameter) =
    value?.canBeAppliedToType(parameter) ?: parameter.type.isMarkedNullable
...
private fun getUnaryParameterConstructor(input: Any?, parameter: KParameter): KFunction<Any> = try {
    ...
    throw IllegalArgumentException(
        "Couldn't create type ${parameter.type.jvmErasure.simpleName} from value $input${parentheticalClassName(input)}",
        e
    )
}

private fun parentheticalClassName(value: Any?) = value?.let { " (${valueClassName(value)})" } ?: ""
private fun valueClassName(value: Any?) = value?.let { value::class.simpleName }
...
private fun canApplyValuesToParameters(values: List<Any?>, parameters: List<KParameter>) = values.indices.all { i ->
    isValidValueForParameter(values[i], parameters[i])
}
